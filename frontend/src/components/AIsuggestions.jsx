...
function AIsuggestions() {
  let [suggested_tickets, setSuggestedTickets] = useState([]);
  ...
  async function getSuggestions() {
    ...
    try {
      ...
      setSuggestedTickets(data.suggested_tickets);
      setLoading(false);
    } catch (e) {
  ...
  if (!suggested_tickets || suggested_tickets.length === 0) {
    return (
      <div className="p-4 bg-white rounded border border-slate-100 shadow-sm flex justify-center items-center text-slate-500">
        <p className="text-sm text-center">
          No ticket recommendations at the moment. Tickets raised by policies
          will appear here.
        </p>
      </div>
    );
  ...
  function handleReject(removeIndex) {
    alert("Removed!");
    const newTickets = suggested_tickets.filter(
      (_, index) => index !== removeIndex,
    );
    setSuggestedTickets(newTickets);
  }
      ...
      <div className="gap-6 flex overflow-x-auto scrollbar-hide  ">
        {suggested_tickets.map((ticket, index) => (
          <div
            key={ticket.title}
            className={
              "bg-white relative p-4 rounded border-slate-100 shadow-sm flex-shrink-0 w-100 w-80 min-h-[250px] flex flex-col justify-between"
            }
          >
            ...
            <div className="bg-slate-50 p-3 rounded-lg mb-2 border border-slate-100">
              <p className="text-[10px] font-bold tracking-wider text-[#158173]">
                Suggested Action:{" "}
              </p>
            ...
            <div className="bg-[#f3f0ff]/50 p-3 rounded-lg mb-4 border border-[#e9e3ff]/40">
              <p className="text-[10px] font-bold italic tracking-wider text-[#6366f1]">
                Reasoning:{" "}
              </p>
